...
def run_model_parametrized(
    ...
    for value in parameter_values:
        ...
        for k in keep_top_classes:
            logging.info(
                f"Parameter {parameter_name} = {value}, Number of classes: {k}"
            )
            ...
            logging.info(
                f"Balanced accuracy: {balanced_accuracy[0]*100:.2f}% ± {balanced_accuracy[1]*100:.2f}% ({elapsed_time:.3f} seconds) ({datetime.now().strftime('%d/%m/%y %H:%M:%S')})"
            )
            df.loc[len(df.index)] = [
                model_name,
                k,
...
def build_df_embedding_type(nodes_embedded, node_types):
    df = pd.DataFrame()
...
def keep_top_classes_only(df_embedding_type, keep_top_classes):
    df_embedding_type = df_embedding_type.copy()
    if keep_top_classes > 0:
        # show only the top k types by number of nodes and group the rest as 'others'
        node_type_counts = df_embedding_type["node_type"].value_counts()
        ...
        df_embedding_type["node_type"] = df_embedding_type["node_type"].apply(
            lambda x: top_counts.index(x) if x in top_counts else keep_top_classes
        )
    return df_embedding_type
...
def calculate_balanced_accuracy(
    ...
    if keep_top_classes > 0:
        ...
        # df['node_type'] = df['node_type'].apply(lambda x: x if x in node_type_counts.index else -1)
        df["node_type"] = df["node_type"].apply(
            lambda x: top_counts.index(x) if x in top_counts else keep_top_classes
        )
    ...
    if df["node_type"].value_counts().min() == 1:
        ...
        print(
            "The class with the lowest number of nodes has more than 1 node. Using StratifiedShuffleSplit"
        )
    ...
    nodes_embedded_train_test = df[
        [f"embedding{i}" for i in range(number_of_dimensions)]
    ].to_numpy()
    ...
    for train_indices, test_indices in SplitterClass(
        ...
        train_x, test_x = (
            nodes_embedded_train_test[train_indices],
            nodes_embedded_train_test[test_indices],
        )
        train_y, test_y = (
            node_types_train_test[train_indices],
            node_types_train_test[test_indices],
        )
        ...
        test_accuracies.append(balanced_accuracy_score(test_y, model.predict(test_x)))
...
def calculate_balanced_accuracy_grape(
    nodes_embedded, node_types, graph, random_state=42, keep_top_classes=0, n_splits=5
):
    ...
    if keep_top_classes > 0:
        ...
        top_counts = [
            ...
            for index, _ in sorted(
                enumerate(zip(counts, node_type_names)),
                key=lambda x: x[1],
                reverse=True,
            )[:keep_top_classes]
        ]
    ...
    for train_indices, test_indices in SplitterClass(
        n_splits=n_splits, test_size=0.3, random_state=random_state
    ).split(nodes_embedded_numpy, node_types_numpy):
        model = DecisionTreeClassifier(max_depth=5)

        train_x, test_x = (
            nodes_embedded_numpy[train_indices],
            nodes_embedded_numpy[test_indices],
        )
        train_y, test_y = (
            node_types_numpy[train_indices],
            node_types_numpy[test_indices],
        )
...
def edge_prediction_pipeline(
    ...
    for i in range(number_of_holdouts):
        ...
        negative_train_size = int(
            math.ceil(
                train_graph_filtered.get_number_of_directed_edges()
                * model._training_unbalance_rate
            )
        )
        negative_test_size = int(
            math.ceil(
        ...
        negative_graph = graph.sample_negative_graph(
            number_of_negative_samples=negative_train_size + negative_test_size,
        ...
        negative_train_split = (
            negative_train_size / negative_graph.get_number_of_directed_edges()
        )
        negative_train_graph, negative_test_graph = negative_graph.random_holdout(
            train_size=negative_train_split, random_state=random_state
        )
        # check if the negative_train_graph and negative_test_graph have any edges in common
        assert (negative_train_graph & negative_test_graph).get_number_of_edges() == 0

        print(
            f"Negative train graph edges: {negative_train_graph.get_number_of_edges()}"
        )
        print(f"Negative test graph edges: {negative_test_graph.get_number_of_edges()}")
        print(f"Positive test graph edges: {test_graph_filtered.get_number_of_edges()}")

        model._negative_train_graph = negative_train_graph

        if verbose:
            pair_to_predict_str = f"{pair_to_predict[0]}-{pair_to_predict[1]}"
            ...
            df_train_filtered.to_csv(f"train_filtered_{pair_to_predict_str}.csv")
            df_test_filtered = build_triples_df(test_graph_filtered)
            df_test_filtered.to_csv(f"test_filtered_{pair_to_predict_str}.csv")
        ...
        if save_embedding_path is not None:
            # try to load the embedding from the path
            try:
                with open(
                    f"{save_embedding_path}{embedder.model_name().replace(' ', '')}/embedding_holdout_{i}.pkl",
                    "rb",
                ) as f:
                    train_embedding = pickle.load(f)
                    train_embedding = EmbeddingResult.load(train_embedding)
                    logging.info("Embedding loaded from file")
                ...
                logging.info("Storing embedding")
                path = f"{save_embedding_path}{embedder.model_name().replace(' ', '')}/"
                os.makedirs(path, exist_ok=True)
                with open(
                    f"{path}embedding_holdout_{i}.pkl",
                    "wb",
                ) as f:
                    pickle.dump(train_embedding.dump(), f)
        ...
        if save_model_path is not None:
            path = f"{save_model_path}{embedder.model_name().replace(' ', '')}/{model.model_name().replace(' ', '')}/"
            # try to load the model from the path
            try:
                with open(
                    f"{path}{embedder.model_name().replace(' ', '')}_{model.model_name().replace(' ', '')}_{pair_to_predict[0]}-{pair_to_predict[1]}_{i}.pkl",
                    "rb",
                ) as f:
                    model._model_instance = pickle.load(f)
                    model._fitting_was_executed = True
                    # for some reason this in the grape is calculated with the support graph instead of the train graph
                    model._training_graph_density = train_graph.get_density()
                ...
                if save_model_path is not None:
                    ...
                    with open(
                        ...
                        pickle.dump(model._model_instance, f)
        ...
        if verbose:
            ...
            # check if all edges of positive test set are not in the positive train set
            pos_pred_edge_missing = pos_pred.apply(
                lambda row: check_if_in_graph(
            ...
            logging.info(
                f"Are all positive edges present in the positive test set NOT in the positive train set? {~pos_pred_edge_missing.all()}"
            )
            logging.info(pos_pred_edge_presence.value_counts())
            logging.info(pos_pred_edge_missing.value_counts())
            # wait 5 seconds to read the output
            time.sleep(10)
        ...
        labels = [True for _ in range(len(pos_pred))] + [
            False for _ in range(len(neg_pred))
        ]
        preds = pd.concat(
            [
        ...
        f1_score_binary = f1_score(labels, preds)
        logging.info(f"F1 binary score: {f1_score_binary}")
        f1_score_micro = f1_score(labels, preds, average="micro")
        logging.info(f"F1 micro score: {f1_score_micro}")
        f1_score_macro = f1_score(labels, preds, average="macro")
        logging.info(f"F1 macro score: {f1_score_macro}")
        # f1_score_samples = f1_score(labels, preds, average="samples")
        # logging.info(f"F1 samples score: {f1_score_samples}")
        f1_score_weighted = f1_score(labels, preds, average="weighted")
        logging.info(f"F1 weighted score: {f1_score_weighted}")
...
def _new_fit(
    ...
    if self._original_graph:
        print(f"{len(edge_type_features) > 0}")
        if hasattr(self, "_negative_test_graph"):
            logging.info("Avoiding negative test graph edges in the training set")
        if hasattr(self, "_negative_train_graph"):
            logging.info("Using pre-generated negative graph for training")
            negative_graph = self._negative_train_graph
        else:
...
def edge_pred_pairs(
    ...
    columns = [
        ...
        "Training set size",
        "Testing set size",
        "Positive training balanced accuracy",
    ...
    for i, pair_to_predict in enumerate(pairs_to_predict):
        ...
        update_fit(
            edge_pred_model, pair_to_predict if train_on_filtered else None, graph
        )
    ...
    df_results["name"] = (
        f"{embedder.model_name()}-{edge_pred_model.model_name()}"
